...
static double UNI(normal_generator_t* gen) 
{
	return 0.5 * (1 + (double)SHR3(gen)/((double) INT_MIN));
}

static double nfix(int hz, int iz, normal_generator_t* gen)
{
...
static double RNOR(normal_generator_t* gen)
{
	int hz, iz;
 ...
 */
double next_gaussian(normal_generator_t* gen) 
{
	return RNOR(gen);
}
